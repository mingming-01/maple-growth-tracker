...
캐릭터명을 입력하면 NEXON Open API를 통해 메이플스토리 캐릭터 정보를 조회합니다.
...
레벨업 시 경험치 진행도가 초기화되면서 그래프가 급락하는 것처럼 보이는 문제를 별도로 처리하여 실제 성장 흐름을 이해하기 쉽도록 구성했습니다.
...
경험치와 같이 큰 숫자는 AI 분석 결과에서 사람이 빠르게 이해할 수 있도록 한국어 단위로 축약하여 표시합니다.
...
이를 통해 긴 숫자를 그대로 표시하는 대신 성장 규모를 직관적으로 확인할 수 있도록 했습니다.
...
AI는 이미 애플리케이션에서 계산하고 정리한 데이터를 해석하는 역할에 집중합니다.
...
Gemini API의 요청 한도 초과나 일시적인 API 오류가 발생하더라도 캐릭터 정보와 성장 데이터 조회에는 영향을 주지 않도록 AI 분석 기능을 별도의 오류 처리 영역으로 분리했습니다.

AI 분석에 실패한 경우 API의 상세 오류 메시지를 그대로 노출하지 않고 사용자가 이해하기 쉬운 안내 메시지를 표시하도록 처리했습니다.
...
특히 데이터의 계산과 판단을 애플리케이션에서 처리하고, AI는 이미 정리된 데이터를 해석하는 역할에 집중하도록 구성했습니다.
...
이를 통해 다음과 같은 역할을 구분했습니다.

* **API** → 원본 데이터 수집
* **Python / Flask** → 데이터 가공 및 계산
* **JavaScript / Chart.js** → 데이터 표현 및 시각화
* **Gemini API** → 성장 데이터 해석

AI에게 모든 판단을 맡기기보다, **애플리케이션에서 확실하게 계산할 수 있는 부분과 AI가 해석하기 적합한 부분을 분리**하는 방식으로 구현했습니다.
...
Gemini API는 사용량 및 요금제에 따라 요청 한도가 적용될 수 있으며, 한도 초과 시 AI 성장 판독 기능을 일시적으로 사용할 수 없을 수 있습니다.
