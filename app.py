...
def generate_ai_analysis(character, history):
    ...
    try:
        ...
        prompt = f"""
                ...
                - 경험치 수치를 표시할 때는 긴 원본 숫자를 그대로 출력하지 말고 한국어 단위로 축약해 표시해.
                - 1조 이상은 "조", 1억 이상은 "억", 1만 이상은 "만" 단위를 사용해.
                - 큰 숫자는 소수점 첫째 자리까지 표시하고, 소수점 둘째 자리 이하는 반올림해.
                - 예: 27,170,594,872,629 → 약 27.2조
                - 예: 15,234,567,890,123 → 약 15.2조
                - 예: 1,234,567,890 → 약 12.3억
                - 원본 데이터의 정확한 값을 변경하는 것이 아니라, 화면에 표시할 때만 가독성을 위해 축약해.
